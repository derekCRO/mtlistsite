<?php 
	  ...
	  foreach($import_list as $key => $data){
			//print_r($data); exit;
		//자유게시판이 아니거나 먹튀정보교환 게시판이 아니면 현시하지 않기
		$i++;
?>
<tr>
	...
	<a href="#detail">
	  ...
	  <tr>
		...
		<td width="55%" align="left" class="content3" style="cursor:pointer" notice_id="<?php echo $data->idx;?>">
			&nbsp;<strong class="content7" style="<?php echo $data->title_style;?>">
				<?php echo $data->title;?>
			...
			<?php if($_SESSION['user_id'] == $data->write_id){?>
				<a class="content10" href="<?php echo $_SERVER_URL; ?>contents/community/add?idx=<?php echo $data->idx; ?>&notice_type=<?php echo $type; ?>"  >
...
<?php 
	  ...
	  foreach($list as $key => $data){
			//print_r($data); exit;
		//자유게시판이 아니거나 먹튀정보교환 게시판이 아니면 현시하지 않기
		if($data->notice_type != 21  && $data->notice_type != 13
		    && $data->notice_type != 12 && $data->notice_type != 11) continue;
		$i++;
?>
<tr>
